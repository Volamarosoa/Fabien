...
function Administrateur(){
  ...
  const handleSelectItem = (itemKey) => {
    handleShow2();
    const itemDetails = administrateur.find(item => item.id === itemKey);
...
//////// SAVE
  const handleSaveSubmit = async (event) => {
      ...
      try {
        const response = await fetch(url + 'administrateur', {
          method: 'POST',
...
//////// DELETE
  const handleDeleteClick = async (item) => {
	...
	const handleSelectModuleChange = (event) => {
		setSelectedItem({ ...selectedItem, module: event.target.value });
	};
	...
	const handleInputMotDePasseChange = (event) => {
		setSelectedItem({ ...selectedItem, motDePasse: event.target.value });
	};
	...
	useEffect(() => {
		const getModule = async () => {
			try {
				const response = await fetch(url + 'module');
					if (!response.ok) {
						throw new Error('Network response was not ok');
					};
				const data = await response.json();
  ...
  return (
              ...
              <div className="col" >   
                <div className="row">
                  ...
                  <Modal show={show} onHide={handleClose}>
                      ...
                      <Modal.Title>Add Administrateur</Modal.Title>
                      </Modal.Header>
                      <Modal.Body>
                          <form action="" method="" id="insert" onSubmit={handleSaveSubmit}>
	<div className="mb-3"> 
	 	<label className="form-label">id</label> 
	 	<select className="form-control" name="module" id="select-module">
			{region.map((elt) => (
	 	...
	 	<input className="form-control" type="text" name="motDePasse" />
	</div>
	<div className="mb-3"> 
	...
	<div className="mb-3"> 
		...
		<td>{item.email}</td>
              ...
              <Modal show={show2} onHide={handleClose2}>
                  <Modal.Header closeButton>
                      <Modal.Title>Update Administrateur</Modal.Title>
                  </Modal.Header>
                  <Modal.Body>    
	...
	<div className="mb-3"> 
	 	<label className="form-label">Mot De Passe</label> 
	 	<input className="form-control" type="text" name="motDePasse" onChange={handleInputMotDePasseChange} value={selectedItem ? selectedItem.motDePasse:''} />
	</div>
	<div className="mb-3"> 
	 	<label className="form-label">Nom</label> 
